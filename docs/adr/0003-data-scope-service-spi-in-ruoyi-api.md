...
policy 计算两个**动态 scope**(自定义、部门及以下)时,需调 `sdss`(`ISysDataScopeService`)查库把范围展开成部门 id 串。但 policy 所在的 `ruoyi-common-mybatis` **不能依赖业务模块 `ruoyi-system`**(`ISysDataScopeService` 所在处)——这正是现有代码用 SpEL bean 字符串 `@sdss` 做晚绑定的原因。
